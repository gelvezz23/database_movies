...
years.sort().forEach((year) => {
  ...
  files.forEach((file) => {
    ...
    if (file !== file.toLowerCase()) {
      movie_errors.push(
        "Invalid JSON filename format; must be lowercase: " + file
      );
    }
    ...
    for (let i = 0; i < requiredProp.length; i++) {
      ...
      if (Array.isArray(currentProp)) {
        ...
        if (!any) {
          errorsFound = true;
          movie_errors.push(
            fileName + " missing one of the props: " + currentProp.join(",")
          );
        }
    ...
    if (movie.year !== parseInt(year)) {
      ...
      const errorMessage =
        fileName +
        " movie is in the wrong year folder. Found: " +
        movie.year +
        ". Expected: " +
        year;
    ...
    if (path.parse(file).name !== expectedFileName) {
      ...
      const errorMessage =
        fileName +
        " movie name is either wrong or file name is not according to guidelines. Expected: " +
        expectedFileName +
        ".json";
      movie_errors.push(errorMessage);
    }
...
function validatePerson(file, folder) {
  ...
  const requiredProperties = ["name", "birthdate", "birthplace"];

  const checkProperties = requiredProperties.map((prop) =>
    person.hasOwnProperty(prop)
  );
  ...
  if (checkProperties.includes(false)) {
    ...
    const missingProps = checkProperties
      ...
      .join(", ");
    console.warn(
      `${fileName} is missing the required properties: ${missingProps}`
    );
  }
  ...
  if (path.parse(file).name !== expectedFileName) {
    ...
    if (fileBaseName !== withoutMiddleNames) {
      ...
      console.warn(`${fileName} person's name is either wrong or file name is not according to guidelines.
        ...
        ${
          withoutMiddleNames !== expectedFileName
            ? `or ${withoutMiddleNames}.json`
            : ""
        }`);
    }
  }
...
actors.forEach((file) => {
  validatePerson(file, actorsFolder);
});
console.log("actors test complete");

directors.forEach((file) => {
  validatePerson(file, directorsFolder);
});
